...
- [x] **卡片悬停动效与光晕/阴影分主题设计**
  - **描述**：在深色主题（Classic & Galaxy）下，针对有 3D 旋转及放大交互的 Bento 卡片 and Blog 分类卡片，悬停时新增外围边框颜色光晕（Halo/Glow）；在浅色主题（Light Mode）下，屏蔽霓虹光晕，改用与博客文章项类似的物理凸起立体阴影（Raised Shadow），使卡片在暖奶油底色上凸显微光浮雕效果。
  - **完成标记**：`[x]`
  - **Commit ID**：`01171fb` & `6f49166`

- [x] **卡片边框色彩一致性与 Astro 路由脚本绑定优化**
  - **描述**：优化 Bento 卡片与 Blog 分类子栏卡片的悬浮边框，使其在深色/浅色主题下的悬浮边框颜色与对应板块的背景色及光晕完全一致，而非统一显示紫色；同时，将 Bento 卡片的 3D 旋转脚本完全重构并包装至 Astro 的 `'astro:page-load'` 事件中，彻底解决由于 client-side 页面切换导致脚本失效、必须手动刷新页面才能重新加载的问题。
  - **完成标记**：`[x]`
  - **Commit ID**：`ffab4e2`
